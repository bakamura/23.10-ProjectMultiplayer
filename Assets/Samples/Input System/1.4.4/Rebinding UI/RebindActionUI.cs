...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        {
            InputBinding newBind = action.bindings[bindingIndex];
            string oldOverridePath = newBind.overridePath;
            ...
            foreach(InputAction otherAction in action.actionMap.actions)
            {
                ...
                {
                    InputBinding bind = otherAction.bindings[i];
                    if (bind.overridePath == newBind.path) otherAction.ApplyBindingOverride(i, oldOverridePath);
                }
            }
        }
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if(CheckDuplicateBindigns(action, bindingIndex, allCompositeParts))
                        {
                            action.RemoveBindingOverride(bindingIndex);
                            CleanUp();
        ...
        private bool CheckDuplicateBindigns(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            InputBinding newBind = action.bindings[bindingIndex];

            foreach(InputBinding binding in action.actionMap.bindings)
            {
                if (binding.action == newBind.action) continue;

                if(binding.effectivePath == newBind.effectivePath)
                {
                    Debug.Log($"Duplicate Binding Found {newBind.effectivePath}");
                    return true;
                }
            }
            ...
            if (allCompositeParts)
            {
                ...
                {
                    if(action.bindings[i].effectivePath == newBind.overridePath)
                    {
                        Debug.Log($"Duplicate Binding Found {newBind.effectivePath}");
                        return true;
                    }
        ...
        [Tooltip("Optional bool that will Override the action label text with the ActionLabelString")]        
        public bool m_OverrideActionLabel;

        [Tooltip("The name that will Override the text in ActionLabel")]
        [SerializeField]
